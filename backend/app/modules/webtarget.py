        ...
        if "functional" in test_types:
            ...
            for index, page in enumerate(pages):
                ...
                for form in page["forms"]:
                    ...
                    for case in functional_cases(form, page):
                        emit(req, case, "functional", page_artefacts)

                nav = navigation_cases(page)
                if nav:
                    nav_req, _created = upsert_requirement(
                        db, org_id, project_id,
                        f"WEB-{short}{tokens[index]}-NAV",
                        f"Every link on {page.get('final_url') or page.get('url')} must resolve.",
                        ["No link answers 4xx or 5xx."], "functional",
                        {"url": page.get("final_url")},
                        "", priority="medium")
                    requirement_count += 1
                    for case in nav:
                        emit(nav_req, case, "functional", page_artefacts)
