...
| The import leaves a **runnable** project behind | Into a project with **zero** environments (asserted as a precondition, so the void is real): `environment_created` is non-null, exactly one environment exists and it is the one the response names, `auth_type == "none"`, `tls_strict == true`, no secret attached. The name comes from the document (`… (imported)`, within the 100-char column), never the `Imported environment` fallback |
| The derived base URL is **correct**, not merely present | The reconstruction assertion: `base_url + endpoint.path` must reproduce the URL the collection declares — one named endpoint spelled out for a readable failure, then the whole inventory as a **set** against the fixture's own absolute URLs (23 distinct URLs across 37 method+path keys). Plain concatenation on purpose: a helper that trimmed or joined cleverly would repair a wrong base URL and hide the defect. Holds whichever side of the split the importer put `/calendar/v3` on |
| It only ever fills a **void** | A project whose user already created `staging (mine)` (different host, different `calendarId`, `tls_strict: false`) imports the collection: `environment_created` is `null`, still exactly one environment, and its name, base URL, variables and TLS flag are unmoved. A **re-import** into the auto-created project likewise creates nothing, and the later OpenAPI import in the precedence test creates nothing either |
| Credentials in the document stay in the document | On the synthetic `billing-api` collection: `tenantId`/`pageSize` arrive with their values, `authToken`/`apiKey`/`client_secret`/`webhookPassword` arrive as **keys with an empty value**, and the four fake values appear **nowhere** in the serialised API responses. The rule is asserted as a substring match, with `tenantId`/`pageSize` as the negative controls |
| The UI says a run is now possible | After importing on the endpoints page, `endpoints-import-environment-created` shows the created name and base URL — both read back from the API, not hard-coded — and links to `/projects/{id}/environments` |
| An empty picker is never rendered | On a project with no environment, the runs page shows `runs-environment-empty-hint` with a link to the environments page, and `runs-launch-env-select` has count **0** — asserted as absent, not as "empty" |
...
- **`api/types.ts`** — `ImportSpecResult` gained `format`, the flat counters `added`/`updated`/`removed`/`total`, and `enriched`/`enrichment_discarded`; the four original keys keep their names and meanings. `Endpoint` gained the three nullable annotations `ai_description`, `ai_group`, `ai_criticality`. It later gained `environment_created: null | CreatedEnvironment` (`{id, name, base_url}`) — deliberately narrow, because it is a confirmation line, not a second environments API.
...
- **`pages/endpoints.page.ts`** — read-only surfaces for the format badge, the two enrichment counters and the three AI columns. `formatBadgeFor(format)` accepts the value either as `data-format` or as the badge's own text; the reasoning is in `docs/TESTID_REGISTRY.md`. It also exposes the derived-environment line and its link (`endpoints-import-environment-created`, `…-link`).
- **`pages/runs.page.ts`** — the environment picker is now exposed read-only (`environmentSelect`) so a spec can assert its **absence**, alongside `environmentEmptyHint` / `environmentEmptyLink`.
- **`api/projects.repository.ts`** — no new route: `listEnvironments` (capability `view`) is how a derived environment is read back in full. Two pure read helpers joined it: `environmentById` and `environmentVariables` (variables are stored as JSON, so values are normalised to strings before comparison).
- **`api/discovery.repository.ts`** — gained `resolvedUrl(baseUrl, path)` / `resolvedUrls(...)`: plain concatenation, documented as such, because the reconstruction assertion is only meaningful if the helper refuses to be clever.
- **`helpers/postman-collection.ts`** — the oracle grew the derivation surface it needs to stay the oracle: `title`, `baseUrlKey`, `baseUrl`, `variables`, per-request `absoluteUrl` (+ the sorted `absoluteUrls` set), and the two rules re-derived from the contract — `looksLikeCredential(name)` and `suggestedVariables(collection)`. Both fixtures' values are pinned in the fixture-precondition tests, so a swapped file fails loudly.
...
## Addendum — Path-parameter binding is now guarded

A run against a templated endpoint used to send the **template**, not a value:

```
GET https://host/calendars/%7BcalendarId%7D/events?calendarId=example
```

Every path-parameterised case therefore failed on a `404` regardless of what the system under test did, and the value that should have filled the placeholder went out as a query parameter instead. The fix (`_bind_path_params` in `backend/app/modules/execution.py`, mirrored by the Go port) fills each `{name}` from the step's params, falls back to the run context, leaves the placeholder literal when neither has a value, percent-encodes what it substitutes, and **removes the key it consumed** from the query params. `{{var}}` interpolation is a separate, older mechanism and still runs first.

### Why the suite could not see it

Because **an outcome assertion cannot catch this defect.** A `404` is an ordinary test result; a suite that asserts "the run completed" or even "this case failed" is perfectly happy while every path-parameterised case is being sent to a URL that cannot exist. The only place a run states what it actually requested is the **recorded evidence URL** — `req_evidence.url` in `execution.py`, which after a response is `str(resp.request.url)`, the absolute URL as sent. That is the oracle, and reading it is the whole point of the new spec.

### `tests/execution.spec.ts` — `@critical @regression`

Arrangement is the existing `generatedCase` fixture (upload → confirm → import `sample_openapi.yaml` → generate): the sample already declares three templated paths — `/customers/{id}`, `/orders/{id}/cancel`, `/invoices/{id}` — and `sample_requirements_en.md` already carries REQ-007/010/011 naming them, so no new fixture data was needed. The spec then selects **only** the generated cases whose steps hit a templated path, approves those, creates the demo-SUT environment (`:9000` + `/api/v2`, bearer) and runs that subset, which keeps the execute job small.

| What is proven | How |
|---|---|
| The arrangement is real | The imported inventory must contain at least one templated path, and generation must have produced at least one case whose step targets one — both asserted before anything is run, so the spec cannot pass vacuously |
| The value is where the binder looks | Every placeholder of every step under test must have a value in that step's own `request.params` — the premise of precedence rule (1). If it were not there, the spec would be proving something else |
| No template reaches the wire | Blanket scan of every recorded evidence URL: the **path** may contain none of `{`, `}`, `%7B`, `%7D` (both spellings, because a sent request is percent-encoded while a request that died in transport records the raw form) |
| The substitution is the *right* value | Per placeholder: the URL's path is matched against the step's template anchored at the **end** (the environment's base URL contributes a prefix the template knows nothing about), the captured segment is decoded, and it must equal the step's own param value — not merely "something other than a brace" |
| The key is not sent twice | The bound name must **not** appear in the URL's query keys — the `?calendarId=example` half of the symptom is a defect on its own, since it can change how the system under test answers |
| The check is not vacuous | A counter of placeholders actually verified must be `> 0`; a run where every case halted before its templated step fails the test rather than passing empty |

Outcomes are deliberately **not** asserted: the demo SUT answers `404` for an unknown id, which is a legitimate result. The spec is about the request, not the verdict.

### Layer changes

- **`helpers/path-params.ts`** (new) — `isTemplatedPath`, `pathParamNames`, `templateMarkersIn`, `pathOf`, `boundPathValues`, `queryKeysOf`. Pure functions, no assertions (§8, §11). The placeholder regex is copied **character for character** from the backend's `_PATH_PARAM_RE` so the suite cannot drift into accepting a laxer form than the product implements — including its overlap with `{{var}}`, which is faithful: the binder runs after interpolation, so anything still spelled `{{…}}` there is unresolved. `boundPathValues` returns **decoded** segments so a spec compares against the step's own value rather than against one encoder's output; a value carrying an encoded `/` still resolves to exactly one capture.
- **`pathOf` exists for a reason.** The debris scan is confined to the URL's path: a brace inside a *query value* is legitimate test data (generated negatives inject payloads), whereas a brace in the path is the defect. The query half is pinned by name instead, via `queryKeysOf`.
- **`api/types.ts`** — run evidence is typed instead of `Record<string, unknown>`: `EvidenceRequest` (`method`, `url`, `headers`, `body`), `EvidenceResponse`, and `RunEvidence` with the alignment rule written down — evidence is appended in step order, so index `i` is `steps[i]`, and a case that halts at the first failed assertion or transport error (FR-EXE-11) records **fewer** entries than it has steps, never more and never reordered. Both the executor and the review serializer sort steps by `order`, which is what makes that alignment true rather than hopeful.
- **`api/execution.repository.ts`** — no new route; two pure read helpers, following the `discovery.repository.ts` precedent: `evidenceUrls(result)` and `evidenceForStep(result, i)`. The latter returning `undefined` is a legitimate answer (the case halted earlier), not a defect, and the spec treats it as one.

### Dev auto-login — pinned as default-off

`POST /v1/auth/dev-session` hands out a full session with **no credentials** when `TRACEO_DEV_AUTOLOGIN=1` (`TRACEO_DEV_AUTOLOGIN_EMAIL`, default `demo@traceo.sa`); a production node refuses to boot with the flag on (`assert_production_safe`). The suite's own backend is configured normally, so `negative.spec.ts` pins the default: the route answers **404 `not_found`** — asserted on `code` + status through `expectApiError`, never on message text (§6/§11).
